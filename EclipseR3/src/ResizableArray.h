...
public:
	...
	void swap(int index1, int index2);//MULTIPLE OBJECTS	||	SWAP TWO ELEMENTS AT INDEXES
...
/*
 * DESTRUCTOR:
 * Destructor for Deleting Array Associated with
 * ResizableArray
 *
 * Author: Clayton Glenn
 */
...
void ResizableArray<T>::addAt(T object, int index) {
	...
	if (index >= this->size || index < 0) {
		throw std::out_of_range(
				"ArrayList<X>::addAt() : index is out of range");
	}
...
void ResizableArray<T>::replaceAt(T object, int index) {
	...
	if (index < this->size && index >= 0) {
		...
		throw std::out_of_range(
				"ArrayList<X>::replaceAt() : index is out of range");
...
/*
 * REMOVEAT:
 * Mutator Function: Removes an object at a
 * specified index
 *
 * Author: Clayton Glenn
 */
...
void ResizableArray<T>::removeAt(int index) {
	...
	if (index >= this->size) {
		throw std::out_of_range(
				"ArrayList<X>::removeAt() : index is out of range");
	}
...
/*
 * DOUBLECAPACITY:
 * Private Mutator Method: Doubles the capacity
 * of the resizable array
 *
 * Author: Clayton Glenn
 */
...
/*
 * HALFCAPACITY:
 * Private Mutator Method: Halves the capacity
 * of the resizable array
 *
 * Author: Clayton Glenn
 */
...
/*
 * SWAP:
 * Public Mutator Method: This function swaps
 * two objects at specified indexes.
 *
 * Author: Clayton Glenn
 */
template<class T>
